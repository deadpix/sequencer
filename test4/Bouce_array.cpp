...
static uint8_t (*dig_read)(uint8_t);

void ArrBounce::init_ptrfunc(uint8_t (*rd_func)(uint8_t)){
...
void ArrBounce::init_ArrBounce(uint8_t* pin_arr, unsigned long interval_millis, int size, uint8_t (*rd_func)(uint8_t)){
	this->size = size;
...
void ArrBounce::attach(uint8_t* pin_arr, uint8_t (*rd_func)(uint8_t)) {
	dig_read = rd_func;
...
//	bool readState = digitalRead(pin[idx]);
	bool readState = dig_read(pin[idx]);
